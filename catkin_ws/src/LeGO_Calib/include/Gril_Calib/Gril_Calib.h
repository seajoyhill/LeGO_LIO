...
struct Ground_Normal_Cost_IL {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    Ground_Normal_Cost_IL(QD Lidar_wrt_ground_, QD IMU_wrt_ground_) :
            Lidar_wrt_ground(Lidar_wrt_ground_), IMU_wrt_ground(IMU_wrt_ground_) {}

    template<typename T>
    bool operator()(const T *q, T *residual) const {
        const Eigen::Matrix<T, 3, 3> R_GL =
                Lidar_wrt_ground.cast<T>().toRotationMatrix();
        const Eigen::Matrix<T, 3, 3> R_GI =
                IMU_wrt_ground.cast<T>().toRotationMatrix();
        const Eigen::Quaternion<T> q_IL{q[0], q[1], q[2], q[3]};
        const Eigen::Matrix<T, 3, 3> R_IL = q_IL.toRotationMatrix();
        const Eigen::Matrix<T, 3, 1> e3 = Eigen::Matrix<T, 3, 1>::UnitZ();

        // R_GL = R_IL * R_GI, therefore both normals must agree in
        // the LiDAR frame.
        const Eigen::Matrix<T, 3, 1> resi = R_IL * R_GI * e3 - R_GL * e3;
        residual[0] = T(GROUND_FACTOR_) * resi[0];
        residual[1] = T(GROUND_FACTOR_) * resi[1];
        return true;
    }

    static ceres::CostFunction *Create(const QD Lidar_wrt_ground_,
                                       const QD IMU_wrt_ground_) {
        return new ceres::AutoDiffCostFunction<Ground_Normal_Cost_IL, 2, 4>(
                new Ground_Normal_Cost_IL(Lidar_wrt_ground_, IMU_wrt_ground_));
    }
...
struct Ground_Plane_Cost_IL {
    ...
    bool operator()(const T *q, const T *trans, T *residual) const {
        ...
        // Plane-normal constraint. With R_GI (Ground -> IMU), R_IL
        // (IMU -> LiDAR), and R_GL (Ground -> LiDAR), the consistent
        // composition is R_GL = R_IL * R_GI.
        Eigen::Matrix<T, 3, 1> e3 = Eigen::Matrix<T, 3, 1>::UnitZ();    // (0,0,1)
        Eigen::Matrix<T, 3, 1> normal_lidar_pred = R_IL * R_GI * e3;
        Eigen::Matrix<T, 3, 1> normal_lidar_meas = R_GL * e3;
        Eigen::Matrix<T, 3, 1> resi_plane = normal_lidar_pred - normal_lidar_meas;
...
struct Linear_acc_Rot_Cost_without_Gravity {
    ...
    bool operator()(const T *q,const T *b_a, const T *trans, T *residual) const {
        ...
        Eigen::Matrix<T, 3, 1> resi = R_LL0_T * (R_IL * IMU_linear_acc_T - bias_aL
...
public:
    ...
    // Cubic B-spline trajectory fitting parameters.  A value of zero for
    // bspline_ctrl_points selects an adaptive number based on the data size.
    bool bspline_enable = true;
    int bspline_ctrl_points = 0;
    int bspline_degree = 3;
    double bspline_smoothing = 1.0e-4;
    ...
    // Estimate LiDAR linear/angular velocity and acceleration from the
    // smoothed pose trajectory instead of finite-differencing the CV state.
